...
from utils_mix import (
    get_logits, read_label_csv, fix_token_type_shape, collate_bert_batch_from_samples,
    build_common_alignment, generate_pseudo_from_fusion, evaluate_models_dynamic,
    strong_aug_bert_shuffle, strong_aug_gat_dropedge
)

# ------------------ 配置区 ------------------
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

ROOT = r"D:\project\data\PHEME_Dataset"
JSON_ROOT = r"D:\project\data\PHEME_Dataset\original-microblog"
...
# 模型加载
bert_model = BertModelClass(r"D:/bert-base-uncased", num_classes=2).to(DEVICE)
...
def main():
    ...
    test_ids_set = set(str(i) for i in eval_df["id"])
    common_ids_set = set(str(sid) for sid in common_ids)  # 统一转字符串防类型不匹配
    ...
    print(f"📊 Test CSV Total Rows: {len(eval_df)}")
    print(f"🔗 Aligned Common IDs:  {len(common_ids)}")
    print(f"✅ Valid Labels (≠-100): {len([l for l in global_labels if l != -100])}")

    # ========================== Alpha 网格搜索外循环 ==========================
    for alpha_step in range(6, 7):
        ...
        bert_model_stage2 = BertModelClass(r"D:/bert-base-uncased", num_classes=2).to(DEVICE)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for i in range(0, len(train_pool), BATCH_SIZE):
                ...
                # --- C. Forward Strong View (强增强) ---
                # 1. BERT Strong (Text Reversal)
                input_ids_strong = strong_aug_bert_shuffle(input_ids, attn_mask)
                b_logits_strong = get_logits(bert_model_stage2(input_ids_strong, attn_mask, token_type_ids))

                # 2. GAT Strong (DropEdge)
                edge_index_strong = strong_aug_gat_dropedge(graph_batch.edge_index, p=DROP_EDGE_RATE)
                g_logits_strong = get_logits(gat_model_stage2(graph_batch.x, edge_index_strong, graph_batch.batch))
            ...
            # ========================== 新增：每个Epoch结束后评估测试集 ==========================
            # 切换评估模式并禁用梯度计算（节省显存/时间）
            bert_model_stage2.eval()
            gat_model_stage2.eval()
            with torch.no_grad():
                current_test_res, _ = evaluate_models_dynamic(
                    bert_model_stage2, gat_model_stage2, test_indices,
                    global_labels, global_to_bert_idx, global_to_gat_idx,
                    bert_dataset_full, gat_dataset_local, DEVICE, BATCH_SIZE,
                    alpha=current_alpha  # ★ 动态传入当前alpha
                )
            # 打印当前Epoch的测试集结果
            print(
                f"📊 Test (Epoch {epoch}) -> Fusion Acc: {current_test_res['fusion_acc']:.4f} | Macro-F1: {current_test_res['fusion_macro_f1']:.4f}")
            # ==============================================================================
    ...
    print("🌟" * 30 + "\n")
    print(f"\n【混淆矩阵 TP / FP / FN / TN】")
    print(f"   TP = {global_best_test_metrics['tp']}")
    print(f"   FP = {global_best_test_metrics['fp']}")
    print(f"   FN = {global_best_test_metrics['fn']}")
    print(f"   TN = {global_best_test_metrics['tn']}")
