...
def evaluate_models_dynamic(bert_model_local, gat_model_local, indices, global_labels, g2b, g2g,
                            bert_dataset_full, gat_dataset_local, device, batch_size=16, alpha=0.5):
    ...
    # 🔧 1. 严格预过滤：仅保留标签有效且数据完整的索引
    valid_indices = []
    dropped_log = []
    for idx in indices:
        if global_labels[idx] == -100:
            dropped_log.append(f"idx_{idx}: label=-100")
            continue
        # 检查数据是否存在（兼容你的Dataset返回None逻辑）
        b_sample = bert_dataset_full[g2b[idx]]
        g_sample = gat_dataset_local[g2g[idx]]
        if b_sample is None or g_sample is None:
            dropped_log.append(f"idx_{idx}: missing_data")
            continue
        valid_indices.append(idx)

    print(f"📊 评估集过滤: 原始 {len(indices)} -> 有效 {len(valid_indices)} | 丢弃 {len(dropped_log)}")
    if dropped_log: print(f"🗑️  丢弃样本明细: {', '.join(dropped_log[:5])}{'...' if len(dropped_log) > 5 else ''}")

    if len(valid_indices) == 0:
        return {'fusion_acc': 0.0, 'fusion_macro_f1': 0.0, 'tp': 0, 'fp': 0, 'fn': 0, 'tn': 0}, 0

    all_preds = []
    all_trues = []
    b_correct = g_correct = 0
    ...
    with torch.no_grad():
        for i in range(0, len(valid_indices), batch_size):
            ...
            # BERT Batch
            b_samples = [bert_dataset_full[g2b[idx]] for idx in batch_idx]
            input_ids, attn_mask, token_type_ids, _ = collate_bert_batch_from_samples(b_samples)
            if input_ids is None: continue
            token_type_ids = fix_token_type_shape(token_type_ids)
            ...
            # GAT Batch
            g_samples = [gat_dataset_local[g2g[idx]] for idx in batch_idx]
            graph_batch = GraphBatch.from_data_list(g_samples).to(device)
            g_out = gat_model_local(graph_batch.x, graph_batch.edge_index, graph_batch.batch)

            # Labels & Predictions
            true_labels = torch.tensor([global_labels[idx] for idx in batch_idx], dtype=torch.long).cpu()
            b_probs = F.softmax(get_logits(b_out), dim=1).cpu()
            g_probs = F.softmax(get_logits(g_out), dim=1).cpu()
            fusion_probs = alpha * b_probs + (1.0 - alpha) * g_probs

            b_pred = b_probs.argmax(dim=1)
            g_pred = g_probs.argmax(dim=1)
            f_pred = fusion_probs.argmax(dim=1)

            b_correct += (b_pred == true_labels).sum().item()
            g_correct += (g_pred == true_labels).sum().item()
    ...
    labels_all = torch.cat(all_trues).numpy()
    final_preds = torch.cat(all_preds).numpy()

    # 🔧 2. 强制固定混淆矩阵维度
    cm = confusion_matrix(labels_all, final_preds, labels=[0, 1])
    tn, fp, fn, tp = cm.ravel()

    # 强校验（若失败说明数据流仍有泄漏）
    assert tn + fp + fn + tp == len(labels_all), f"❌ 混淆矩阵不一致: {tn + fp + fn + tp} vs {len(labels_all)}"
    ...
    p_cls, r_cls, f1_cls, _ = precision_recall_fscore_support(labels_all, final_preds, labels=[0, 1], zero_division=0)

    # 🔧 3. 修复原代码中 bert_acc/gat_acc 硬编码为 0.0 的Bug
    valid_n = len(labels_all)
    return {
        'bert_acc': b_correct / valid_n,
        'gat_acc': g_correct / valid_n,
        'fusion_acc': fusion_acc,
        'fusion_macro_f1': fusion_macro_f1,
        'true_p': p_cls[0], 'true_r': r_cls[0], 'true_f1': f1_cls[0],
        'false_p': p_cls[1], 'false_r': r_cls[1], 'false_f1': f1_cls[1],
